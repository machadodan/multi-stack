...
def nova_oportunidade(sender, instance, **kwargs):
    if instance.status == 2:
        ...
        send_mail(assunto, corpo_email, email_remetente, lista_email, html_message=html_message_diarista)
...
def password_reset_token_created(sender, instance, reset_password_token, *args, **kwargs):
    ...
    email_rementente = 'projetostreinaweb@gmail.com'
    assunto = "Email para resetar sua senha no e-diaristas"
    corpo_email = ''
    email_destino = [reset_password_token.user.email, ]
    send_mail(assunto, corpo_email, email_rementente, email_destino, 
    html_message=html_message_reset)

# comentado para não enviar e-mails
