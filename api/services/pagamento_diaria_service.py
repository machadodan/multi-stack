...
from .diaria_service import atualizar_staus_diaria, listar_diaria_id
...
def realizar_pagamento(diaria, card_hash):
    ...
    if transacao['status'] == 'paid':
        ...
        return
         ...
         valor=diaria.preco, transacao_id=transacao['tid'],
         diaria=diaria)
         atualizar_staus_diaria(diaria.id, 1)
...
def cancelar_pagamento(diaria_id, penalidade):
     ...
     if cancelamento['refunded_amount'] != float (valor * 100):
        raise serializers.ValidationError("Erro ao estornar pagamento")
